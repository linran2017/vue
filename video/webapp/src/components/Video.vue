...
export default {
  name: 'video',
    ...
    methods:{
        dataLoad(){
            //获得标签
            this.axios.get('http://laravel.linran136.com/api/tags').then((response) => {
                this.tags=response.data.date;
            })
            //获取课程，如果有tid就获得标签对应的tid,如果没有就获得所有课程
            let tid=this.$route.params.tid;
            this.axios.get('http://laravel.linran136.com/api/lesson/'+(tid?tid:0)).then((response) => {
                console.log(response);
                this.lesson=response.data.date;
            })
        }
    }
}
